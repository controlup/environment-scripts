...
foreach ($q in $iq){
	foreach ($item in $q){
        ...
        #controlup demo\euc environments\controlup.cloud.com\delivery groups\citrixcloudazure
        #      0              1                  2                   3               4   
        #determine EUC path
        #ToLower() to avoid having to deal with casing issues
        $local_originalPath = $global:OriginalPaths[$item.sName]
        $local_eucPath = $local_originalPath.Split("\")[2].tolower() 
        #Count how many machines we found by EUC environment. 
        # In case getting the list of machines happens before extension connection state changes.
        # IE at 9:58 some extension disconnects
        # At 10:00 we get the list of machines. This list doesn't include <some extension>
        # At 10:01 some extension reconnects.
        # At 10:02 we determine if extensions are connected.
        # All extensions appear to be connected but the list of machines is missing all <some extension> machines
        # Causing Build-CUTree to delete them. And the "prevent disconnected extensions from getting deleted" won't save them machines
        $machinesByEUC[$local_eucPath] += 1
...
(Invoke-CUQuery -Fields "Name","Path" -Scheme "Main" -Table "Folders" -Take $maxValue -Focus "$root\EUC Environments").data|%{$global:eucFList.add($_)|out-null}
...
foreach ($f in $eucConCheck){
	$local_eucPath = $f.Split("\")[2].tolower()
    $hasMachines = $machinesByEUC[$local_eucPath] -gt 0
	
	if($VerbosDebug){Write-CULog -Msg "$($local_eucPath) has machines? $($hasMachines) ($($machinesByEUC[$local_eucPath]))" -ShowConsole -color Green}
	else{Write-CULog -Msg "$($local_eucPath) has machines? $($hasMachines) ($($machinesByEUC[$local_eucPath]))"}
    
    if (($EC|?{$_.toLower() -like "*$($f.toLower())*"}) -and $hasMachines)
    {$global:eucConnected.Add($f)|out-null}else{$global:eucDisconnectedMsg.Add($f)|out-null}
}
        ...
        if ($Delete){
	        ...
	        foreach ($CUFolder in $($CUFolderSyncRoot.Path)){
    	        ...
    	        ##if (($CUFolder -notin $ExtFolderPaths.FolderPath) -and ($CUFolder -ne $("$CURootFolder"))){ #prevents excluding the root folder
						$skip = $false
						foreach ($path in $global:eucDisconnected){if ($CUFolder -like "$path*"){$skip = $true;write-host "NOT deleting $CUFolder";break}}
	        ...
	        foreach ($CUComputer in $CUComputers.Where{$_.path.toLower() -like "$CURootFolder$CUSyncFolder*".toLower()}){
    	            ...
    	            if (!($ExtTreeHashTable[$CUComputer.name].name)){
						...
						foreach ($path in $global:eucDisconnected){
							if ($CUComputerPath -like "$path*"){$skip = $true;write-host "NOT deleting $CUComputerPath";break}
						}
